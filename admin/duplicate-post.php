...
class DuplicatePost {
	...
	function add_actions() {
		...
		add_action( 'admin_action_duplicate_post_save_as_new_post_draft', array(
			$this,
	...
	function add_nofollow_noindex_to_clones() {
		if ( ! $this->get_option( "duplicate_post_add_nofollow_noindex" ) ) {
			return;
		}
	...
	public function remove_default_post_actions_for_cloned_docs() {
		...
		if ( $pagenow == 'post.php' && isset( $_GET['action'] ) && $_GET['action'] == 'edit' && isset( $_GET['post'] ) ) {
			...
			if ( $original_post_id ) {
                ...
                <style type="text/css">
                    #duplicate-action, #delete-action, #publishing-action {
                        display: none;
                    }
                </style>
		...
		if ( $pagenow == 'post.php' && isset( $_GET['action'] ) && $_GET['action'] == 'edit' && isset( $_GET['post'] ) ) {
			...
			if ( $original_post_id ) {
                ...
                <div id="publishing-action-update">
                    ...
                    <a class="button" href="<?php echo esc_url( get_permalink( $original_post_id ) ); ?>">Go to original
                        post</a>

					<?php if ( $allow_merge_back || $allow_submit_for_review ):
						$diff_url = add_query_arg( array(
							'page' => 'show-diff',
					...
					<?php if ( $allow_submit_for_review ): ?>
                        ...
                        <input name="submit_for_review" type="submit"
                               class="button button-primary button-large<?php echo $class; ?>" id="submit_for_review"
                               value="<?php echo $submit_label; ?>">

					<?php endif ?>

					<?php if ( $allow_merge_back ): ?>
                        <input name="merge_back" type="submit" class="button button-primary button-large"
                               id="merge_back" value="<?php echo $merge_label; ?>">

                        <input name="save_as_new" type="submit" class="button save_as_new button-primary button-large"
                               id="save_as_new" value="Save as New Post">
                        <input name="save_as_new_id" type="hidden" id="save_as_new_id"
                               value="<?php echo esc_attr( $original_post_id ); ?>">
					<?php endif ?>
                </div>
                <style type="text/css">
                    #update-waitng {
	...
	public function register_my_custom_submenu_page() {
		add_submenu_page( '', 'Show Differences', 'Show Differences', 'read', 'show-diff', array(
			$this,
			'show_diff_callback'
		) );
	}
	...
	private function get_side_by_side_diff( $post_id, $original_post_id ) {
		...
		?>
        <style type="text/css">
            .ChangeReplace, .Differences.DifferencesSideBySide {
                max-width: 100%;
            }
	...
	private function my_acf_get_fields_in_group( $group_id ) {
		...
		if ( isset( $acf_group_fields[ $group_id ] ) ) {
			return $acf_group_fields[ $group_id ];
		}
	...
	function duplicate_post_add_duplicate_post_button() {
		...
		if ( isset( $_GET['post'] ) && $allowed ) {
            ...
            <div id="duplicate-action">
                <a style="width: 100%;text-align: center;margin-bottom: 10px;"
                   class="submitduplicate duplication button"
                   href="<?php echo DuplicatePost::duplicate_post_get_clone_post_link( $_GET['post'] ) ?>">
					<?php _e( 'Duplicate and Edit', 'dem' ); ?>
                </a>
				<?php if ( $backup ) { ?>
                    <input name="revert_back_to_cloned" type="submit"
                           class="button revert_back_to_cloned button-primary button-large" id="revert_back_to_cloned"
                           value="Ooops. Re-link to Original Post">
                    <input name="unlink_post_forever" type="submit" class="button button-primary button-large"
                           id="unlink_post_permanently"
                           title="This will unlink the post from the original. There is no undo after clicking this button"
                           value="Unlink from Original Post Forever">
                    <style type="text/css">
	...
	function duplicate_post_copy_post_meta_info( $new_id, $post ) {
		...
		foreach ( $meta_keys as $meta_key ) {
			if ( in_array( $meta_key, array( "_dp_original", "_dp_submited" ) ) ) {
				continue;
			}
